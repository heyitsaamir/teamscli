...
Every Teams bot needs an Azure Active Directory (AAD) app registration. This page explains how teams2 creates and manages AAD apps.

## Why TDP Creates AAD Apps

teams2 creates AAD apps through the Teams Developer Portal (TDP) API (`/aadapp/v2`), **not** through the Microsoft Graph API directly.
...
TDP returns an `id` field for the newly created AAD app, but this is **not** the same as the Graph API's object ID. When teams2 needs to perform Graph operations (like creating a client secret), it must:
...
AAD apps created by teams2 use:
...
The full flow when teams2 creates a new app:
...
```bash
teams2 app auth secret create <appId>
```

To write credentials to a `.env` file:

```bash
teams2 app auth secret create <appId> --env .env
```
