---
title: "app bot migrate"
command: "teams2 app bot migrate"
---

<!-- AUTOGENERATED:START - Do not edit between these markers -->
# teams2 app bot migrate

Migrate bot from BF tenant to Azure

## Usage

```
teams2 app bot migrate [appId] [options]
```

## Arguments

| Argument | Description | Required |
|----------|-------------|----------|
| `appId` | App ID | No |

## Options

| Flag | Description |
|------|-------------|
| `--subscription <id>` | [OPTIONAL] Azure subscription ID |
| `--resource-group <name>` | Azure resource group (required) |
| `--create-resource-group` | [OPTIONAL] Create the resource group if it doesn't exist |
| `--region <name>` | [OPTIONAL] Azure region for resource group (default: westus2) |
<!-- AUTOGENERATED:END -->

## Details

Migrates a bot from the BF (Bot Framework) tenant to Azure Bot Service. This is needed when you want to use features that require Azure, such as OAuth connections or SSO.

### What Happens

1. **Validation** — confirms the bot is currently in the BF tenant and checks Azure prerequisites
2. **Dry run** — runs an ARM deployment with `--what-if` to validate before making changes
3. **Delete BF registration** — removes the bot from the BF tenant
4. **Create Azure Bot** — deploys an Azure Bot resource in your resource group
5. **Rollback** — if step 4 fails, automatically re-registers the bot in BF with the original configuration

### What Stays the Same

- AAD app registration (same `CLIENT_ID`)
- Client secrets (same `CLIENT_SECRET`)
- Tenant ID (same `TENANT_ID`)
- The bot ID used in the Teams manifest

### What Needs Reconfiguration

After migration, you may need to manually reconfigure:

- **M365 Extensions channel** — must be re-enabled in Azure Portal
- **Calling endpoint** — must be set again if previously configured

### Examples

```bash
teams2 app bot migrate <appId> --resource-group my-rg
```

Create the resource group if it doesn't exist:

```bash
teams2 app bot migrate <appId> --resource-group my-rg --create-resource-group --region eastus
```

### Prerequisites

- Azure CLI (`az`) installed and logged in
- An Azure subscription
- Bot must currently be in the BF tenant (check with `teams2 app bot status`)

See [Bot Locations](/concepts/bot-locations) for more on why and when to migrate.
