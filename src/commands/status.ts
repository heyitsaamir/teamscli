...
export const statusCommand = new Command("status")
  ...
  .action(wrapAction(async (options: StatusOptions) => {
    ...
    if (!tdpToken) {
      ...
      tdpSpinner.warn({ text: `TDP: ${pc.yellow("not connected")} ${pc.dim("(token unavailable)")}` });
    } else {
      ...
      try {
        ...
        if (tenantEnabled && userAllowed) {
          ...
          if (!silent) {
            logger.info(`  ${pc.dim("Your user policy allows sideloading, but the tenant-wide setting is off.")}`);
            logger.info(`  ${pc.dim("Ask your admin to enable it in Teams Admin Center →")}`);
            logger.info(`  ${pc.dim("Org-wide app settings → Custom apps → \"Allow interaction with custom apps\"")}`);
          }
        } else if (tenantEnabled && !userAllowed) {
          tdpSpinner.warn({ text: `Sideloading: ${pc.yellow("not allowed for your account")}` });
          if (!silent) {
            logger.info(`  ${pc.dim("Sideloading is enabled for the tenant, but your user policy blocks it.")}`);
            logger.info(`  ${pc.dim("Ask your admin to update your Teams app setup policy to allow custom app uploads.")}`);
          }
        } else {
          tdpSpinner.warn({ text: `Sideloading: ${pc.yellow("disabled")}` });
          if (!silent) {
            logger.info(`  ${pc.dim("Neither the tenant nor your user policy allows sideloading.")}`);
            logger.info(`  ${pc.dim("Ask your admin to enable it in Teams Admin Center →")}`);
            logger.info(`  ${pc.dim("Org-wide app settings → Custom apps → \"Allow interaction with custom apps\"")}`);
          }
        }
