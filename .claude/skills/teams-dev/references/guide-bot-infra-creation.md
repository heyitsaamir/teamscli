...
**If NO endpoint yet - Quick devtunnel setup:**

```bash
devtunnel create --allow-anonymous
devtunnel port create -p 3978 --protocol auto    # Use port 3978 with protocol auto
devtunnel host                                    # Keep running in separate terminal
```

**Important:** Use `--protocol auto` (not `https`) and port `3978`.

Get endpoint URL: `devtunnel show` → use the URL + `/api/messages`

Alternative: [ngrok](https://ngrok.com) or see [full devtunnel docs](https://learn.microsoft.com/en-us/azure/developer/dev-tunnels/get-started?tabs=windows)
