...
**Checkpoint:** After creating the project, verify the `.env` file contains all three required credentials:
...
**Checkpoint:** Verify the bot project's `.env` file contains all three required credentials:
- `CLIENT_ID` is present
- `CLIENT_SECRET` is present
- `TENANT_ID` is present
...
devtunnel port create -p 3978 --protocol auto    # Use port 3978 with protocol auto
devtunnel host                                    # Keep this running
```

**Important:** Use `--protocol auto` (not `https`) and port `3978`.

Get the public URL with `devtunnel show` (e.g., `https://xxxxx-3978.region.devtunnels.ms`)
